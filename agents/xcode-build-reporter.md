...
4. **Read warnings**: Call `mcp__xcode__GetBuildLog` with `severity: "warning"` to get warnings. Use the `glob` parameter to filter to relevant source files if needed.
...
### Discovering Build Parameters

Before running xcodebuild, discover the project configuration:

1. **Find workspace or project**: Look for `.xcworkspace` or `.xcodeproj` in the project directory. Prefer `.xcworkspace` when both exist.
   ```
...
2. **List available schemes**:
   ...
   Or for a project:
   ```
   xcodebuild -list -project "<project>" 2>&1
   ```
...
4. If discovery fails, ask the parent agent for the workspace, scheme, and destination.
...
- If the Xcode MCP tool is unavailable, fall back to xcodebuild CLI and note this in the report.
- If the build produces no output at all, report status as FAILURE with an error indicating an empty build log.
- If there are linker errors (not tied to a specific file/line), format them as: `[Linker] — Error message here`
- If there are build system errors (e.g., missing provisioning, scheme not found), include them as errors with the prefix `[Build System]`.
