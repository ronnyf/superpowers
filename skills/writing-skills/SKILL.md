...
| Reports less than it found (a reviewer, auditor, or search that self-filters) | Ask for everything, filter in a separate pass | Scope-narrowing instructions ("only report high-severity issues", "be conservative") |

**Why prohibitions backfire on shaping problems:** under a competing incentive ("make the prompt self-contained"), agents negotiate with "don't X". In head-to-head wording tests on dispatch-prompt guidance, the prohibition arm produced clearly more of the unwanted content than the recipe arm (fully separated distributions), and trended worse than even the no-guidance control — micro-test your own case rather than assuming, but never reach for the prohibition by default. A recipe leaves nothing to negotiate: the output matches the stated shape or it doesn't.

**Why an under-reporting instruction costs more than it buys:** "only report high-severity issues", "be conservative", "don't flag nits" read as calibration and execute as a quota. Anthropic documents this for Claude Opus 5 directly: "If your review prompt says 'only report high-severity issues' or 'be conservative,' the model may follow that instruction literally and report less; ask it to report everything and filter in a separate pass instead." The asymmetry is what makes it expensive — a false positive costs one line to dismiss, while a finding the producer suppressed leaves no trace, so no later pass can weigh what it dropped. Split the jobs: ask the producer for everything it found plus whatever routes each item (a concrete failure scenario, a severity, a `file:line`), and filter where the whole set is visible.

**Rules for whichever form you pick:**
- **No nuance clauses.** "Don't X unless it matters" reopens the negotiation — appending a single nuance clause to a winning recipe degraded it from consistent to noisy in the same wording tests. Express a real exception as its own conditional on an observable predicate.
- **Exemption clauses don't scope.** "This limit doesn't apply to code blocks" still suppresses code blocks. If part of the output must be exempt, restructure so the rule can't reach it.
- **Emphasis is not enforcement.** `CRITICAL`, `MUST FOLLOW`, `ALWAYS`, `NEVER`, `IMPORTANT`, `do NOT` — the capitalised register is doing no work the plain statement of the rule isn't already doing, and it crowds out the thing that does: why the rule exists and what breaks without it. State the mechanism and let the rule follow from it. *Unmeasured as a register claim* — what is documented is the adjacent one, that Claude Opus 5 "performs best when given the complete task specification up front and left to run," so over-prescribed step sequences and compliance rituals cost quality. Treat the capitals as a symptom worth reading: reaching for them usually means the rule's reason is missing.
